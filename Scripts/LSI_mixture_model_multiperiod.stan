...
data {
  ...
  //Site observations
  int<lower = 1, upper = N_Periods> Period[N_Sites]; //what period is the site from?
  //Specimen observations
  ...
  int<lower = 1, upper = N_Dimensions> Dimension[N_Measurements]; //which dimension is this?
...
parameters {
  ...
  vector<lower = 0>[8] period_sigma;
  matrix[8, N_Periods] z_period;
  cholesky_factor_corr[8] L_Rho_period;
  //site-level parameters
  vector<lower = 0>[8] site_sigma;
  matrix[8, N_Sites] z_site;
  cholesky_factor_corr[8] L_Rho_site;
  //interaction between anatomy and site
...
transformed parameters {
  ...
  matrix[N_Periods, 8] v_period;
  matrix[8, 8] Rho_period;
  ...
  v_period = (diag_pre_multiply(period_sigma, L_Rho_period) * z_period)';
  ...
  for(i in 1:N_Sites) {
    site_theta_raw[1, i] = grand_theta_raw[1] + v_period[Period[i], 1] + v_site[i, 1];
    site_theta_raw[2, i] = grand_theta_raw[2] + v_period[Period[i], 2] + v_site[i, 2];
    for(j in 1:N_Element_Portions) {
      theta_raw[1, i, j] = grand_theta_raw[1] + v_period[Period[i], 1] + v_site[i, 1] + v_element[j, 1] + v_interaction[N_Element_Portions * (i - 1) + j, 1];
      theta_raw[2, i, j] = grand_theta_raw[2] + v_period[Period[i], 2] + v_site[i, 2] + v_element[j, 2] + v_interaction[N_Element_Portions * (i - 1) + j, 2];
      mu_female[i, j] = grand_mu_female + v_period[Period[i], 3] + v_site[i, 3] + v_element[j, 3] + v_interaction[N_Element_Portions * (i - 1) + j, 3];
      logdelta_immature[i, j] = grand_logdelta_immature + v_period[Period[i], 4] + v_site[i, 4] + v_element[j, 4] + v_interaction[N_Element_Portions * (i - 1) + j, 4];
      logdelta_male[i, j] = grand_logdelta_male + v_period[Period[i], 5] + v_site[i, 5] + v_element[j, 5] + v_interaction[N_Element_Portions * (i - 1) + j, 5];
      logsigma_immature[i, j] = grand_logsigma_immature + v_period[Period[i], 6] + v_site[i, 6] + v_element[j, 6] + v_interaction[N_Element_Portions * (i - 1) + j, 6];
      logsigma_female[i, j] = grand_logsigma_female + v_period[Period[i], 7] + v_site[i, 7] + v_element[j, 7] + v_interaction[N_Element_Portions * (i - 1) + j, 7];
      logsigma_male[i, j] = grand_logsigma_male + v_period[Period[i], 8] + v_site[i, 8] + v_element[j, 8] + v_interaction[N_Element_Portions * (i - 1) + j, 8];
    }
  }
...
generated quantities {
  ...
  //period-level parameters
  vector[N_Periods] period_theta_raw[2];
  vector[N_Periods] period_theta[3];
  vector[N_Periods] period_p_immature;
  vector[N_Periods] period_theta_female;
  vector[N_Periods] period_mu_immature;
  vector[N_Periods] period_mu_female;
  vector[N_Periods] period_mu_male;
  vector[N_Periods] period_sigma_immature;
  vector[N_Periods] period_sigma_female;
  vector[N_Periods] period_sigma_male;
  //site-level parameters
  ...
  //period-level parameter estimates
  for(i in 1:N_Periods) {
    period_theta_raw[1, i] = grand_theta_raw[1] + v_period[i, 1];
    period_theta_raw[2, i] = grand_theta_raw[2] + v_period[i, 2];
    period_mu_female[i] = grand_mu_female + v_period[i, 3];
    period_mu_immature[i] = period_mu_female[i] - exp(grand_logdelta_immature + v_period[i, 4]);
    period_mu_male[i] = period_mu_female[i] + exp(grand_logdelta_male + v_period[i, 5]);
    period_sigma_immature[i] = exp(grand_logsigma_immature + v_period[i, 6]);
    period_sigma_female[i] = exp(grand_logsigma_female + v_period[i, 7]);
    period_sigma_male[i] = exp(grand_logsigma_male + v_period[i, 8]);
  }
  for(i in 1:N_Periods) {
    period_theta[1, i] = inv_logit(period_theta_raw[1, i] + log(0.5));
    period_theta[2, i] = (1 - period_theta[1, i]) * inv_logit(period_theta_raw[2, i] + log(1));
    period_theta[3, i] = 1 - sum(period_theta[1:2, i]);
    period_p_immature[i] = period_theta[1, i];
    period_theta_female[i] = period_theta[2, i] / sum(period_theta[2:3, i]);
  }
  //site-level parameter estimates
  for(i in 1:N_Sites) {
    site_mu_female[i] = grand_mu_female + v_period[Period[i], 3] + v_site[i, 3];
    site_mu_immature[i] = site_mu_female[i] - exp(grand_logdelta_immature + v_period[Period[i], 4] + v_site[i, 4]);
    site_mu_male[i] = site_mu_female[i] + exp(grand_logdelta_male + v_period[Period[i], 5] + v_site[i, 5]);
    site_sigma_immature[i] = exp(grand_logsigma_immature + v_period[Period[i], 6] + v_site[i, 6]);
    site_sigma_female[i] = exp(grand_logsigma_female + v_period[Period[i], 7] + v_site[i, 7]);
    site_sigma_male[i] = exp(grand_logsigma_male + v_period[Period[i], 8] + v_site[i, 8]);
  }
